...
/**
 * Generated class for the AdddessertPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-adddessert',
  templateUrl: 'adddessert.html',
})
export class AdddessertPage {
  imageurl = "";
  imagecheck= false;
  ...
  ionViewDidLoad() {
    ...
    addfod(name,price,des){
      ...
      if(name.length > 0 && price.length > 0 && des.length > 0 && this.imageurl.length > 0){
        ...
        this.db.list("fods/dessert").push({
          name:name,
          price:price,
          des:des,
          image:this.imageurl,
